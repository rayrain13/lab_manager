<template>
  <el-card>
    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column prop="user_name" label="预约人" width="110" />
...
import { listReservations, approveReservation, rejectReservation } from '../../api/reservation'
...
async function handleApprove(row) {
  await ElMessageBox.confirm(`确定通过「${row.user_name}」对「${row.lab_name}」的预约吗？`, '审核', { type: 'warning' })
...
async function handleReject(row) {
  ...
  await rejectReservation(row.id, { review_comment: value || null })
